  ...
  return (
    <div className="border-b border-rule/70 py-3 last:border-b-0">
      <div className="grid grid-cols-[minmax(4.5rem,9rem)_minmax(0,1fr)_auto] items-center gap-3 sm:gap-4">
        <span className="truncate text-base text-ink-600">
          {tally.label}
        </span>
        <span className="h-2.5 min-w-0 bg-sheet-3">
          <span className="block h-full rounded-r-[4px] bg-tide-400" style={{ width: `${share * 100}%` }} />
        </span>
        <span className="flex w-[4.5rem] items-center justify-end gap-1.5 font-mono text-sm tabular-nums text-ink-900">
          {tally.papers}
...
export function QueryPanel({ data }: { data: LandingData }) {
  ...
  return (
    <section id="query" className="scroll-mt-28 border-t border-rule/60 bg-sheet-1 md:scroll-mt-20">
      <div className="mx-auto max-w-7xl px-5 py-14 sm:px-8 sm:py-20">
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end lg:gap-12">
          <SectionHeader
            ...
            lede="Answers are counted over structured records, never generated prose. Open any count to see every paper and stored source excerpt behind it."
          />
          <div className="border-l-2 border-tide-400 pl-5">
            <p className="text-3xl font-semibold text-ink-900">{limitations.length}</p>
            <p className="mt-1 max-w-[20ch] text-base leading-snug text-ink-600">reported theme families in this fixed query</p>
          </div>
        </div>
        ...
        <Panel className="mt-8 overflow-hidden sm:mt-10">
          ...
          <div className="p-5 sm:p-6">
            <p className="mb-4 max-w-[72ch] text-base leading-relaxed text-ink-600">
              Source-linked candidate extractions across the {totals.processedPapers}{" "}
              papers processed so far. Only fields carrying an{" "}
            ...
            {limitations.length > 0 ? (
              <div>
                {visible.map((tally) => (
                  <TallyRow key={tally.key} tally={tally} max={max} of={totals.processedPapers} />
                ))}
                {remaining.length > 0 && (
                  <details className="group/more border-b border-rule/70">
                    <summary className="cursor-pointer list-none py-3 text-base font-medium text-tide-600 [&::-webkit-details-marker]:hidden">
        ...
        <p className="mt-6 max-w-[75ch] text-base leading-relaxed text-ink-400">
          Counts describe stored candidate extractions, not which method is better or which problem
          matters most. Frequency in this corpus is not a scientific ranking.
        </p>
      </div>
    </section>
  );
