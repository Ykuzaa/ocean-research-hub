...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        result = {
            "status": "VALID_NOT_IMPORTED", "workbook_sha256": book.sha256,
            "papers": len(book.paper_index), "claims": len(book.claims),
            "field_definitions": len(book.field_contract),
            "research_gap_candidates": len(book.research_gaps), "warnings": book.warnings,
        }
    else:
        result = {"status": "IMPORTED", **CorpusRepository(args.database).import_workbook(book).as_dict()}
